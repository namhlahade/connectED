...
//    @Binding var calendars: Set<EKCalendar>?
    ...
    func makeUIViewController(context: UIViewControllerRepresentableContext<CalendarChooser>) -> UINavigationController {
      let chooser = EKCalendarChooser(selectionStyle: .multiple, displayStyle: .allCalendars, entityType: .event, eventStore: eventService.eventStore)
        chooser.selectedCalendars = eventService.calendars ?? []
    ...
    class Coordinator: NSObject, UINavigationControllerDelegate, EKCalendarChooserDelegate {
        ...
        func calendarChooserDidFinish(_ calendarChooser: EKCalendarChooser) {
          parent.eventService.calendars = calendarChooser.selectedCalendars
